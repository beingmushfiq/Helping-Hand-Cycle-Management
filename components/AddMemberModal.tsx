...
import { SearchIcon, UserIcon, UserPlusIcon } from './Icons';
...
export const AddMemberModal: React.FC<AddMemberModalProps> = ({ allUsers, currentMemberIds, onClose, onAddMembers }) => {
  ...
  const availableUsers = useMemo(() => {
    ...
    return allUsers.filter(user => !user.cycleId && user.email !== 'admin@gmail.com' && !currentMemberSet.has(user.uid));
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4" aria-modal="true" role="dialog" aria-labelledby="add-member-title">
      <div 
        ref={modalContentRef} 
        className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-lg m-4 flex flex-col max-h-[90vh]"
        onClick={e => e.stopPropagation()}
      >
        <h2 id="add-member-title" className="text-2xl font-bold mb-6 text-slate-900 dark:text-white">Add New Members</h2>
        
        <form onSubmit={handleSubmit} className="flex-grow flex flex-col overflow-hidden">
          <div className="flex-grow flex flex-col overflow-hidden">
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
              Selected: <span className="font-bold text-primary-600 dark:text-primary-400">{selectedUserIds.size}</span>. The cycle will be extended by {selectedUserIds.size} {selectedUserIds.size === 1 ? 'month' : 'months'}.
            </p>
              ...
              {filteredUsers.length > 0 ? (
                ...
                <div className="text-center py-10">
                  ...
                  <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                    {availableUsers.length === 0 ? "No registered users available to add." : "No users match your search."}
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="mt-6 flex justify-end space-x-3 flex-shrink-0">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md shadow-sm hover:bg-primary-500 disabled:bg-slate-400 dark:disabled:bg-slate-600"
            >
